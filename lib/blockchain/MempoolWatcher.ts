  ...
  public startListening() {
    ...
    const unwatchBlocks = getEthWsClient().watchBlocks({
      ...
      onBlock: (block: any) => {
        ...
        console.log(`[ETH L1] Captured Real Block: ${block.number} | Transactions: ${block.transactions.length}`);
        ...
        block.transactions.forEach((tx: any) => {
           ...
           const type = valueEth > 5 ? 'whale' : 'dust'; // strictly defined mathematically
      ...
      onError: (error: any) => console.error("[Block Watch Error] Critical WebSocket Failure:", error)
    });
