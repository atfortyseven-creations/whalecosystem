 ...
 *      Genesis allocation: 5,000,000 QDs to the Sovereign Vault wallet.
...
contract QuantumDots is ERC20, ERC20Burnable, ERC20Pausable, AccessControl, ERC20Permit, ERC20Votes {
    ...
    /// @dev Sovereign Vault wallet — receives the 5,000,000 QD genesis allocation.
    address public constant SOVEREIGN_VAULT = 0x78831C25c86eA2a78A6127fC2Ccb95E612D87b4a;

    /// @dev Genesis allocation to the Sovereign Vault at deploy time.
    ...
    {
        require(defaultAdmin != address(0), "QuantumDots: zero admin");

        _grantRole(DEFAULT_ADMIN_ROLE, defaultAdmin);
        _grantRole(PAUSER_ROLE, defaultAdmin);
        _grantRole(MINTER_ROLE, defaultAdmin);

        // Genesis mint: 5,000,000 QDs directly to the Sovereign Vault.
    ...
    function mint(address to, uint256 amount) public onlyRole(MINTER_ROLE) {
        require(totalSupply() + amount <= MAX_SUPPLY, "QuantumDots: 210M hard cap reached");
        _mint(to, amount);
    }
