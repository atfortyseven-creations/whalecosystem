...
export function useSystemSignOut() {
    ...
    const nuclearDisconnect = useCallback(async () => {
        ...
        // useSystemAccount reads this flag on mount and skips system_session_v2 restore.
        // This is the single most important line: it prevents any hook on the page
        // from re-authenticating during the async cleanup that follows.
        try {
            sessionStorage.setItem('__disconnected__', '1');
            // Remove session-wallet keys right away so the cookie-poll in
            // useSystemAccount stops returning isConnected:true.
        ...
        // The system_handshake cookie is polled every 1s by useSystemAccount.
        // If it still exists when wagmi disconnects, the hook immediately returns
        // isConnected:true and the connect page redirects back to /portfolio.
        // Clearing cookies FIRST eliminates that race condition.
        try {
        ...
        try {
            ...
            Object.keys(localStorage).forEach(key => {
                ...
                if (lower.includes('whale-system-wallet-registry-v3')) return; // Preserve vault
        ...
        try {
            ...
            sessionStorage.setItem('__disconnected__', '1'); // Re-write after clear
        ...
        // STEP 5 — Disconnect Wagmi (after all storage is already cleared).
        // Now safe: even if wagmi fires React state updates during disconnect,
        // there are no cookies or storage left to trigger re-authentication.
        try {
            if (disconnectAsync) {
                await Promise.race([
                    disconnectAsync(),
        ...
        // STEP 7 — Lock local wallet vault (keeps encrypted blob, just wipes keys from memory).
        try {
        ...
        // STEP 8 — Hard redirect to home. Wipes all in-memory React state.
