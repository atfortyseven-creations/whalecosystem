...
import { SovereignSignalOverlay } from './SovereignSignalOverlay';
...
interface PreCognitiveMatrixProps {
    symbol: string;
}

export function PreCognitiveMatrix({ symbol }: PreCognitiveMatrixProps) {
    const [state, setState] = useState<PrecognitiveState | null>(null);
    const [isHovered, setIsHovered] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [showSovereignOverlay, setShowSovereignOverlay] = useState(false);
    ...
    if (!state) {
        return (
            <div className="bg-[#FAF9F6] border border-[#050505]/10 rounded-2xl p-6 h-40 flex items-center justify-center animate-pulse">
                <span className="text-[10px] font-black uppercase text-[#050505]/30 tracking-widest">Warming Matrix Engine...</span>
            </div>
        );
    }
    ...
    return (
        <>
            <AnimatePresence>
                {showSovereignOverlay && (
                    <SovereignSignalOverlay 
                        symbol={symbol}
                        ...
                        onClose={() => setShowSovereignOverlay(false)}
                    />
                )}
            </AnimatePresence>
