      ...
      <motion.div className="flex gap-20 will-change-transform w-max" animate={{ x: [0, -2000] }} transition={{ duration: 40, repeat: Infinity, ease: "linear" }}>
        {content.map((item: string, i: number) => (
          <span key={i} className="text-[10px] font-mono uppercase tracking-[0.4em] font-bold text-white/30 flex items-center gap-3">
            <span className="w-1 h-1 rounded-full bg-teal-500 shadow-[0_0_8px_rgba(20,184,166,0.5)]" />
            {item}
          </span>
        ))}
      </motion.div>
...
export default function WhaleAlertLanding() {
  ...
  return (
    <main className="relative min-h-screen bg-[#050505] text-white selection:bg-teal-500/30 overflow-x-hidden">
      
      {/* ── BACKGROUND LAYERS: Inhuman Fidelity ───────────────────────────────── */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        {/* Layer 1: Corporate Cube Grid (Subtle Depth) */}
        <div 
          className="absolute inset-0 opacity-[0.03] mix-blend-screen bg-repeat"
          style={{ 
      ...
      <nav className="fixed top-0 left-0 w-full z-[100] px-6 py-8 flex justify-between items-center bg-gradient-to-b from-black/80 to-transparent backdrop-blur-sm">
        <div className="flex items-center gap-4 group cursor-pointer">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[9px] font-mono text-white/40 tracking-[0.3em] uppercase mt-1">Sovereign Terminal</span>
          </div>
        </div>
        <div className="flex items-center gap-8">
          <button onClick={handleEntry} className="text-[11px] font-mono font-bold tracking-[0.2em] uppercase text-white/50 hover:text-white transition-colors">
            {address ? "Go to Terminal" : "Initialize Auth"}
          </button>
          ...
          <UltraFluidLayer style={{ y, opacity, scale }} className="flex flex-col items-center text-center max-w-6xl z-10">
            ...
            <Reveal delay={0.2}>
              <div className="inline-flex items-center gap-3 px-4 py-2 bg-white/5 border border-white/10 rounded-full mb-10 backdrop-blur-xl">
                 ...
                 <span className="text-[10px] font-mono font-black tracking-[0.25em] text-white/70 uppercase">Institutional Pipeline v3.0</span>
              </div>
            </Reveal>
      ...
      {/* ── FLOW SECTION: Olas Hokusai Parallax ───────────────────────────────── */}
      <section className="relative py-40 border-t border-white/5">
        <div 
          className="absolute inset-0 z-0 opacity-20 transition-opacity grayscale hover:grayscale-0 duration-1000"
          style={{ 
           ...
           <div className="grid lg:grid-cols-2 gap-20 items-center">
              <div>
                ...
                <div className="space-y-8">
                  {[
                    { title: "Real-Time Whale Stream", desc: "SIMD-0109 priority fee monitoring on Solana and L1/L2 EVM mempool capture." },
                    { title: "Sovereign Mesh Network", desc: "Encoded P2P data propagation with ZK-proven signal authenticity." },
                    { title: "Dormant Capital Tracking", desc: "Automated forensic analysis of movement in deep-cold wallets." }
                  ].map((item, i) => (
                    <Reveal key={i} delay={0.1 * i} className="group p-8 bg-white/[0.02] border border-white/5 rounded-3xl hover:bg-white/[0.04] hover:border-white/10 transition-all">
                      <h4 className="text-xl font-black mb-2 flex items-center gap-4">
                        <span className="text-teal-500 font-mono text-sm">0{i+1}</span>
                        {item.title}
                      </h4>
              ...
              <div className="relative aspect-square">
                 ...
                 <div className="relative h-full w-full border border-white/10 rounded-[4rem] overflow-hidden backdrop-blur-3xl bg-black/40 p-12">
                    <div className="h-full w-full border border-white/5 rounded-[3rem] p-8 flex flex-col justify-between">
                       <Activity className="text-teal-500 w-12 h-12" />
                       <div className="space-y-4">
                          <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                             <motion.div className="h-full bg-teal-500" animate={{ x: ["-100%", "100%"] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} />
                          </div>
                       ...
                       <div className="font-mono text-[10px] text-white/20 tracking-widest uppercase">
                          Telemetry Processing System v3
                       </div>
                    </div>
                 </div>
              </div>
      ...
      <section className="relative min-h-[80vh] flex flex-col items-center justify-center p-6 border-t border-white/5 overflow-hidden">
        ...
        <Reveal className="text-center relative z-10">
          ...
          <button onClick={handleEntry} className="px-16 py-8 bg-teal-500/10 border-2 border-teal-500/50 text-teal-400 font-black uppercase tracking-[0.3em] text-[11px] rounded-full hover:bg-teal-500 hover:text-black transition-all hover:scale-105 active:scale-95 shadow-[0_0_40px_rgba(20,184,166,0.2)]">
              ...
              className="bg-[#0A0A0A] border border-white/10 p-12 rounded-[3rem] max-w-2xl w-full shadow-[0_0_100px_rgba(0,0,0,0.8)] relative overflow-hidden">
               ...
               <div className="relative z-10">
                 <h2 className="text-2xl font-black uppercase tracking-tighter mb-8 flex items-center gap-4">
                    <WhaleLogo className="w-10 h-10" /> Initialize Handshake
                 </h2>
                 <div className="space-y-6 text-[12px] text-white/40 leading-relaxed font-mono tracking-wider mb-12">
                   <p>This process will verify your wallet ownership via a gasless digital signature. No private keys are ever shared with the network.</p>
                   <p className="p-6 bg-white/[0.03] rounded-2xl border border-white/5">
                      "I hereby acknowledge that the Sovereign Terminal is a research-grade intelligence protocol. I am responsible for all cryptographic key management."
                   </p>
                 </div>
