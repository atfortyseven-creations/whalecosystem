...
export function InstitutionalPortfolioView() {
    ...
    return (
        <div className="flex flex-col relative text-black selection:bg-black/10:bg-white/10 min-h-[85vh] bg-[#fdfcf9]">
            
            {/* Fine Grid Background */}
            <div className="absolute inset-0 pointer-events-none opacity-[0.05].02]"
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col relative z-20">
            <section className="px-8 pt-20 pb-12 flex flex-col items-center text-center">
                {address && (
                    <button onClick={clearWallet} className="absolute top-6 right-6 p-2.5 bg-white border border-black/10 rounded-xl shadow-sm hover:bg-rose-50:bg-rose-900/20 text-rose-500 transition-all">
                        <Trash size={16} />
                    </button>
                )}
                
                <button onClick={onNetworkClick} className="bg-white border border-black/10 px-5 py-2 rounded-full shadow-sm hover:border-black/20:border-white/20 transition-all flex items-center gap-2.5 mb-8">
                ...
                {address && (
                    <div className="flex flex-col items-center gap-3">
                        <button onClick={copy} className="bg-black/5 border border-black/10 px-6 py-3 rounded-2xl shadow-sm hover:shadow-md transition-all flex items-center gap-3 group">
                            <code className="text-[12px] font-mono font-bold text-black/70 group-hover:text-black:text-white">{truncate(address, 16)}</code>
                            {copied ? <Check size={14} className="text-emerald-500" /> : <Copy size={14} className="opacity-30 group-hover:opacity-100" />}
                        </button>
                        <a href={`${scannerBase}/address/${address}`} target="_blank" rel="noopener noreferrer" className="text-[9px] font-bold uppercase tracking-widest text-black/30 hover:text-black:text-white transition-colors flex items-center gap-1.5">
                                ...
                                <div className="divide-y divide-black/5">
                                    {transactions.map((tx: any) => (
                                        <a 
                                            ...
                                            className="flex items-center justify-between p-7 hover:bg-black/[0.02]:bg-white/[0.02] transition-colors group"
                                        >
        ...
        <motion.button
            ...
            className={`flex flex-col items-start p-6 rounded-[2rem] border transition-all text-left group disabled:opacity-30 disabled:cursor-not-allowed ${
                highlighted 
                    ...
                    : 'bg-white text-black border-black/10 hover:border-black/30:border-white/30'
            }`}
        >
            <div className={`p-3 rounded-xl mb-4 transition-colors ${highlighted ? 'bg-white/10 text-white' : 'bg-black/5 text-black group-hover:bg-black:bg-white group-hover:text-white:text-black'}`}>
                <Icon size={20} />
            </div>
    ...
    return (
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="flex flex-col max-w-2xl mx-auto w-full pt-12 pb-20 px-6">
            <div className="flex items-center justify-between mb-12">
                ...
                <button onClick={onBack} className="w-10 h-10 rounded-full bg-black/5 hover:bg-black/10:bg-white/10 flex items-center justify-center text-black transition-all">
                    <X size={20} />
                </button>
            </div>
        ...
        <ModalView title="Send Assets" subtitle="Transfer" icon={<ArrowUpRight />} onBack={onBack}>
            <div className="space-y-8">
                <FormField label={`Destination Address (${networkInfo.name})`}>
                    <input type="text" value={toAddress} onChange={e => setToAddress(e.target.value)} placeholder="0x..." className="w-full bg-black/5 border-none p-5 rounded-2xl font-mono text-sm outline-none placeholder:opacity-30 focus:ring-2 ring-black/10 text-black transition-all" />
                </FormField>
                <FormField label="Amount">
                    <div className="relative">
                        <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" className="w-full bg-black/5 border-none p-5 rounded-2xl font-mono text-3xl font-black outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none focus:ring-2 ring-black/10 text-black transition-all" />
            ...
            <div className="flex flex-col items-center gap-10">
                ...
                <div className="w-full text-center">
                    <button onClick={() => { navigator.clipboard.writeText(address); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className="w-full group">
                        <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-black/50 block mb-3 group-hover:text-black:text-white transition-colors">Your Address</span>
                        <div className="bg-black/5 p-5 rounded-xl font-mono text-sm text-black flex items-center justify-between transition-all hover:bg-black/10:bg-white/10">
    ...
    return (
        <ModalView title="Scan Address" subtitle="Camera" icon={<Scan />} onBack={onBack}>
             <div className="p-10 bg-black/5 rounded-3xl text-center border-2 border-dashed border-black/10">
                ...
                <input type="text" placeholder="Or paste address directly" onChange={e => onResult(e.target.value)} className="mt-8 w-full bg-white border border-black/10 p-4 rounded-xl font-mono text-sm outline-none focus:border-black/30:border-white/30 text-black transition-all" />
             </div>
        </ModalView>
    );
}
        ...
        <ModalView title="Select Network" subtitle="Blockchain Environment" icon={<Globe />} onBack={onBack}>
            <div className="grid grid-cols-1 gap-3">
                {Object.entries(NETWORKS).map(([id, data]) => (
                    <button key={id} onClick={() => setNetwork(id as NetworkId)} className={`flex items-center justify-between p-6 rounded-2xl border transition-all ${activeNetwork === id ? 'bg-black text-white border-transparent' : 'bg-white text-black border-black/10 hover:bg-black/5:bg-white/5'}`}>
