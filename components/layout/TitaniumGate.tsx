...
export function TitaniumGate({ children }: TitaniumGateProps) {
    ...
    const initialIsPublicPage = ['/', '/connect', '/docs', '/terms', '/privacy', '/developers', '/forum', '/careers', '/news'].some(
    ...
    const isPublicPage = ['/', '/connect', '/docs', '/terms', '/privacy', '/developers', '/forum', '/careers', '/news'].some(
        path => path === pathname || (path !== '/' && pathname?.startsWith(path))
    );
