...
/**
 * QuantumDotsPanel — Full sovereign QDs interface
 * ─────────────────────────────────────────────────────────────────────────────
 * Features:
 *   • Live on-chain QD balance (ERC-20 read, 15s refresh)
 *   • Quantum Transfer terminal (ERC-2612 Permit + transferWithReceiptPermit)
 *   • On-chain receipt history (reads last N receipts from QuantumLedger)
 *   • Animated 256-bit quantum entropy visualizer
...
function buildAdvancedMetadata(msTimestamp: number, routeId: string): `0x${string}` {
    try {
        return encodeAbiParameters(
            ...
            ['QuantumLedger/v2', BigInt(msTimestamp), keccak256(toBytes(routeId)) as `0x${string}`]
        );
    } catch { return '0x'; }
}
...
// ── Live Entropy Tab ──────────────────────────────────────────────────────────
function EntropyTab() {
    const [entropy, setEntropy] = useState<bigint>(() => generateQuantumEntropy());
    const [autoRefresh, setAutoRefresh] = useState(true);
    const intervalRef = useRef<NodeJS.Timeout | null>(null);

    useEffect(() => {
        if (autoRefresh) {
            intervalRef.current = setInterval(() => setEntropy(generateQuantumEntropy()), 3000);
        } else {
    ...
    return (
        <div className="space-y-6 p-6">
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="font-black text-sm uppercase tracking-tight">256-Bit Quantum Entropy</h3>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        onClick={() => setEntropy(generateQuantumEntropy())}
            ...
            <div className="bg-amber-50 border border-amber-200/60 rounded-2xl p-4">
                <div className="flex items-start gap-3">
                    ...
                    <p className="text-[9px] font-mono text-amber-700 leading-relaxed uppercase tracking-wide">
                        Each QD transfer is permanently bound to a unique 256-bit entropy value generated client-side using the browser's cryptographically secure PRNG. This entropy is ABI-encoded alongside route metadata and stored immutably on-chain in the QuantumLedger contract — making every transfer cryptographically unique and unforgeable.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-5 p-6">
            ...
            <div className="border border-black/8 rounded-2xl overflow-hidden">
                ...
                {[
                    ...
                    { label: 'QuantumLedger',      addr: LEDGER_ADDR },
                ].map(({ label, addr }) => (
            ...
            <div className="bg-[#FAFAF8] border border-black/8 rounded-2xl p-5 space-y-3">
                ...
                <div className="space-y-2">
                    {[
                        ...
                        { k: 'Receipt',      v: 'Immutable on-chain QuantumLedger' },
                    ].map(({ k, v }) => (
...
function FullReceiptCard({ receipt, onClose }: { receipt: OnChainReceipt; onClose: () => void }) {
    ...
    const rows = [
        ...
        { icon: <Zap size={10} />,      label: 'Quantum Entropy',  value: `0x${receipt.quantumEntropy.toString(16).padStart(64,'0')}`, key: 'ent', mono: true },
                ...
                try {
                    ...
                    if (
                        ...
                        fetched.push({
                            ...
                            quantumEntropy: raw.quantumEntropy,
                ...
                <div className="flex items-center justify-between mb-2">
                    <div>
                        ...
                        <div className="text-[9px] font-mono text-black/40 mt-0.5">
                            {userReceiptCount.toString()} on-chain receipts · QuantumLedger
                        </div>
                    </div>
                    <button
        ...
        try {
            ...
            try {
                ...
                txHash = await writeContractAsync({
                    ...
                    args: [recipient as `0x${string}`, parsedAmount, memo || '', deadline, v, r, s, quantumEntropy, advancedMetadata],
            ...
            setAmount('');
            setMemo('');
            toast.success('Transfer confirmed on-chain', { description: 'Receipt minted in QuantumLedger.' });
        } catch (err: any) {
        ...
        <>
            <div className="space-y-5 p-6">
                ...
                <p className="text-[8px] font-mono text-black/25 leading-relaxed uppercase tracking-wider text-center">
                    This tx injects 256-bit quantum entropy, ABI-encoded route metadata and generates an immutable Keccak-256 fingerprint stored on-chain in the QuantumLedger. ERC-2612 Permit eliminates separate approve(). Fees are negligible L2 costs.
                </p>
            </div>
...
export function QuantumDotsPanel() {
    ...
    if (!address) {
        return (
            <div className="w-full rounded-3xl border border-black/8 bg-white/60 backdrop-blur-3xl p-10 flex flex-col items-center gap-4 text-center">
                ...
                <p className="text-sm text-black/40 max-w-xs">Connect a wallet to access the Quantum Dots transfer terminal and view your on-chain receipts.</p>
            </div>
        );
    }
        ...
        <div className="w-full rounded-3xl border border-black/8 bg-white/70 backdrop-blur-3xl overflow-hidden shadow-sm">
            ...
            <div className="bg-[#FAF9F6] px-7 pt-7 pb-6 border-b border-black/5">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    {[
                        { icon: <Zap size={8} />,       label: 'ERC-2612 Permit'  },
                        { icon: <Atom size={8} />,       label: 'Quantum Entropy'  },
                        { icon: <Lock size={8} />,       label: 'MEV Protected'    },
