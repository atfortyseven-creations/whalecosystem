...
export default function PolymarketGlassDashboard({ embedded = false }: { embedded?: boolean }) {
    ...
    // [MAGISTRAL] Fetch Alpha Signals
    useState(() => {
        const fetchAlpha = async () => {
             try {
                const res = await fetch('/api/polymarket/alpha');
                const data = await res.json();
                if (data.signals) setAlphaSignals(data.signals);
             } catch (e) {
                console.warn("Alpha fetch failed", e);
             }
        };
        fetchAlpha();
    });
        ...
        >
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="hidden md:flex items-center space-x-2">
                                ...
                                <span className="text-sm font-medium tracking-wide text-white/90">
                                    {isPolygon ? "Whale Alert Live" : "Terminal Mode"}
                                </span>
                            </div>
                        </div>
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <motion.div variants={itemVariants} className="lg:col-span-8 space-y-8">
                    ...
                    {/* [MAGISTRAL] Alpha Signals Area */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {alphaSignals.slice(0, 4).map((signal, i) => (
                            <motion.div 
                                key={i}
                                initial={{ opacity: 0, scale: 0.95 }}
                                animate={{ opacity: 1, scale: 1 }}
                                transition={{ delay: i * 0.1 }}
                                className="p-5 rounded-3xl bg-white/5 border border-white/10 hover:border-indigo-500/50 transition-all group overflow-hidden relative"
                            >
                                ...
                                <div className="flex items-center gap-2 mb-3">
                                    <div className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter ${
                                        signal.conviction === 'High' ? 'bg-indigo-500 text-white' : 'bg-blue-500/20 text-blue-400'
                                    }`}>
                                ...
                                <h4 className="text-sm font-black text-white line-clamp-2 mb-3 leading-tight group-hover:text-indigo-400 transition-colors">
                                    {signal.market.question}
                                </h4>
                                <div className="flex items-center justify-between">
                                    <div className="space-y-1">
                                        <div className="text-[8px] font-black text-gray-500 uppercase tracking-widest">Smart Money Exposure</div>
                                        <div className="text-lg font-black text-white italic tracking-tighter">
                                            {signal.smartMoneyExposure?.toFixed(1)}%
                                        </div>
                                    </div>
                                    <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center bg-black/20 group-hover:border-indigo-500/50 transition-all">
                            ...
                            {!isProxyEnabled && (
                                <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 backdrop-blur-md flex items-start space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-white/60 mt-1 mb-2">You need a Secure Proxy to operate on the Terminal.</p>
                                        <button
                                            onClick={login}
