...
// ─── Lazy Panels ───────────────────────────────────────────────────────────────
const WhaleDashboard     = lazy(() => import('./WhaleDashboard'));
const WatchlistTable     = lazy(() => import('./WatchlistTable').then(m=>({default:m.WatchlistTable})));
const AlertsPanel        = lazy(() => import('./AlertsPanel').then(m=>({default:m.AlertsPanel})));
const LiveTransactions   = lazy(() => import('./LiveTransactions'));
const InstitutionalChart = lazy(() => import('./InstitutionalChart').then(m=>({default:m.InstitutionalChart})));
const NewsOfToday        = lazy(() => import('./NewsOfToday').then(m=>({default:m.NewsOfToday})));
const EntityGraphVis     = lazy(() => import('./EntityGraphVis').then(m=>({default:m.EntityGraphVis})));
...
function NavItem({ item, active, onClick }: { item: any; active: boolean; onClick: ()=>void }) {
    ...
    return (
        <button
            onClick={onClick}
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-[11px] font-bold transition-all duration-150 uppercase tracking-widest group ${active ? '' : 'hover:bg-[rgba(0,0,0,0.04)]'}`}
            style={{
...
// ─── INLINE MOCKS REMOVED - ZERO MOCK MANDATE ─────────────────────────
...
function renderTabContent(tab: DashboardTab) {
    switch(tab) {
        case 'dashboard':     return <WhaleDashboard />;
        case 'watchlist':     return <WatchlistTable />;
        case 'alerts':        return <AlertsPanel />;
        case 'live-txs':      return <LiveTransactions />;
        case 'multicharts':   return <InstitutionalChart data={[]} />;
        ...
        case 'market-news':   return <NewsOfToday />;
        case 'api':           return <ApiTerminal />;
        case 'security':      return <SecurityScanner />;
        case 'visual-graph':  return <EntityGraphVis />;
