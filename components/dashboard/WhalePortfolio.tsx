    ...
    return (
        <div className="flex flex-col space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: 'Total Whale AUM',   value: whaleLoading ? '…' : fmt(totalAUM), color: '#050505', icon: <DollarSign size={16}/> },
                    { label: 'Whale Txns 30d',    value: whaleLoading ? '…' : totalTxns.toLocaleString(), color: '#050505', icon: <Activity size={16}/> },
                    { label: 'Smart Money Inflow', value: whaleLoading ? '…' : fmt(netFlowsUSD), color: '#00C076', icon: <TrendingUp size={16}/> },
                    { label: 'Avg Alpha Score',   value: whaleLoading ? '…' : avgAlpha.toFixed(1), color: '#D4AF37', icon: <Zap size={16}/> },
                ].map((s, i) => (
                    <div key={i} className="bg-white border border-[#E5E5E5] rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="text-3xl font-black font-mono tracking-tighter" style={{ color: s.color }}>{s.value}</div>
                    </div>
                ))}
            </div>
            ...
            <div className="bg-white border border-[#E5E5E5] rounded-3xl overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-[#E5E5E5] bg-[#FAF9F6] flex items-center gap-4 flex-wrap">
                ...
                {view === 'leaderboard' ? (
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="text-[9px] font-black text-[#888888] uppercase tracking-[0.2em] bg-[#FAF9F6] border-b border-[#E5E5E5]">
