...
export default async function CareerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="relative min-h-screen bg-transparent text-black selection:bg-black/10 font-sans antialiased overflow-x-hidden">
      ...
      <main className="relative z-10 w-full max-w-4xl mx-auto px-5 sm:px-8 pt-24 md:pt-36 pb-24 md:pb-32 flex flex-col">
        ...
        <nav className="mb-10 sm:mb-16">
          <Link 
            href="/careers" 
            className="group inline-flex items-center gap-3 px-5 py-2.5 bg-black/10 border border-black/10 rounded-full font-sans text-[11px] font-bold uppercase tracking-[0.2em] text-black/60 hover:text-white hover:border-white/30 transition-all backdrop-blur-sm"
          >
        ...
        <article className="flex flex-col gap-16 sm:gap-20">
          ...
          <section className="flex flex-col md:flex-row gap-6 md:gap-16">
            <div className="md:w-1/3 shrink-0">
               <div className="flex items-center gap-4 mb-4">
                 <div className="w-10 h-10 rounded-full bg-black/10 flex items-center justify-center text-[10px] font-black font-mono text-black/70">01</div>
                 <h3 className="font-sans text-[20px] sm:text-[24px] font-black tracking-tight text-black">
                   The Role
                 </h3>
               </div>
            </div>
            <div className="md:w-2/3 flex flex-col gap-6 pt-2 md:pt-1">
              {role.paragraphs.map((para, idx) => (
                <p key={idx} className="font-sans text-[15px] sm:text-[16px] text-black/60 leading-[1.8] font-medium">
                  {para}
                </p>
              ))}
            </div>
          ...
          <section className="flex flex-col md:flex-row gap-6 md:gap-16">
            <div className="md:w-1/3 shrink-0">
               <div className="flex items-center gap-4 mb-4">
                 <div className="w-10 h-10 rounded-full bg-black/10 flex items-center justify-center text-[10px] font-black font-mono text-black/70">02</div>
                 <h3 className="font-sans text-[20px] sm:text-[24px] font-black tracking-tight text-black">
                   Requirements
                 </h3>
               </div>
              ...
              <ul className="flex flex-col gap-6">
                {role.requirements.map((req, idx) => (
                    <li key={idx} className="flex items-start gap-5">
                      ...
                      <span className="font-sans text-[15px] sm:text-[16px] text-black/60 leading-[1.7] font-medium">{req}</span>
                    </li>
                ))}
              </ul>
            </div>
        ...
        <section className="mt-20 sm:mt-32 pt-16 sm:pt-20 border-t border-black/10">
          <div className="mb-10 text-center">
            <h2 className="text-[28px] sm:text-[36px] font-black tracking-tight text-black mb-4">Apply for this Position</h2>
            <p className="text-[15px] text-black/50">Submit your credentials. We review applications strictly on technical merit.</p>
          </div>
          <div className="bg-white/5 backdrop-blur-2xl border border-black/10 rounded-3xl p-6 sm:p-10">
