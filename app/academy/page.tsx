...
import { ChevronDown, GraduationCap, FileText, Upload, Download, BookOpen, Lock, Folder } from "lucide-react";
...
function generateTopics(count: number) {
    const topics = Array.from({ length: count }).map((_, i) => {
        const cat = TOPIC_CATEGORIES[i % TOPIC_CATEGORIES.length];
        return {
            id: `m${i+1}`,
            title: `${cat} - Component ${Math.floor(i/7) + 1}`,
            category: cat,
            desc: `Comprehensive coverage of ${cat.toLowerCase()} principles, structured for professional analysts and developers seeking advanced proficiency.`,
            content: `This highly detailed module covers all essential methodologies, technical nuances, and practical implementations related to ${cat}. Upon completing this curriculum, participants will possess a solid grasp of core tenets and advanced strategies.`,
            time: `${45 + (i % 30)} mins`
        };
    });
    // Sort alphabetically — locale-aware for consistent ordering across iOS/Android/Desktop
    return topics.sort((a, b) => a.title.localeCompare(b.title, 'en', { sensitivity: 'base' }));
}

const ALL_MODULES = generateTopics(490);
...
export default function AcademyPage() {
    ...
    return (
        <div className="min-h-screen text-black/80 font-sans pb-0 pt-28 relative overflow-x-hidden">
            <div className="fixed inset-0 pointer-events-none -z-10 bg-[#FAF9F4]" />
