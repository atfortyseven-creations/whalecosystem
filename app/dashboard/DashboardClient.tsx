...
import { Crown, Terminal, Database, Sparkles, LayoutGrid, Flame, Shield, LockKeyhole, Plus, Settings2, ScanLine, Activity } from 'lucide-react';
...
export default function DashboardClient() {
  ...
  const tabs = [
    ...
    { id: 'logs' as const, label: 'Build Logs', icon: Terminal },
    { id: 'scanner' as const, label: 'ZK Scanner', icon: ScanLine },
  ...
  const fetchLogs = async () => {
    try { const res = await fetch('/api/logs?limit=20'); setLogs(await res.json()); } catch(e){}
  };
  ...
  useEffect(() => {
    ...
    fetchVariables();
    fetchLogs();
    const interval = setInterval(() => { if(activeTab === 'logs') fetchLogs() }, 2000);
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tight mb-1 flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-mono bg-white/10 px-2 py-1 rounded-md text-white/50">PROD</span>
                </h1>
              ...
              <AnimatePresence mode="wait">
                {activeTab === 'canvas' && (
                  <motion.div key="canvas" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-8">
                    <div className="flex justify-between items-center mb-8">
                      <h2 className="text-xl font-bold flex items-center gap-2"><LayoutGrid size={20}/> Projects & Markets</h2>
                    ...
                    {isCreating && (
                      <div className="mb-8 p-6 bg-white/5 border border-white/10 rounded-xl flex items-center gap-4">
                        ...
                        />
                        <button onClick={handleCreateProject} className="bg-[#e0ff00] text-black px-4 py-2 rounded-lg text-sm font-bold">Deploy</button>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {projects.map((p) => (
                        <div key={p.id} className="group p-6 bg-white/5 border border-white/10 hover:border-white/30 rounded-xl transition-all relative overflow-hidden">
                          <div className="flex justify-between items-start mb-4">
                            ...
                            <button onClick={() => handleDeleteProject(p.id)} className="text-red-500/50 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">Delete</button>
                          </div>
                          <div className="flex items-center gap-2 mb-4">
                            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                            <span className="text-xs font-mono text-green-400">LIVE</span>
                            <span className="text-xs font-mono text-white/30 ml-auto">Oracle: {p.oracle}</span>
                          </div>
                      ...
                      {projects.length === 0 && !isCreating && (
                        <div className="col-span-2 text-center py-20 text-white/30 font-mono border border-dashed border-white/10 rounded-xl">
                          No active projects found. Deploy a new market to begin.
                        </div>
                      )}
                    </div>
                  </motion.div>
                ...
                {activeTab === 'variables' && (
                  <motion.div key="variables" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-8">
                     <div className="flex justify-between items-center mb-8">
                      <h2 className="text-xl font-bold flex items-center gap-2"><Database size={20}/> Environment Variables</h2>
                    </div>
                    {/* Placeholder for variable editor */}
                    <div className="text-white/40 font-mono text-sm space-y-4">
                        ...
                        {variables.length === 0 && <div className="text-center py-12 border border-dashed border-white/10 rounded-xl">No variables defined globally.</div>}
                    </div>
                  </motion.div>
                )}
                ...
                {activeTab === 'logs' && (
                  <motion.div key="logs" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-0 h-full flex flex-col">
                     <div className="p-4 border-b border-white/10 bg-black/60 flex items-center gap-2 text-xs font-mono text-white/40">
                         <Terminal size={14} /> build.logs
                     </div>
                     <div className="flex-1 overflow-y-auto p-4 space-y-2 font-mono text-xs max-h-[500px]">
                         {logs.map(log => (
                             <div key={log.id} className="flex gap-4">
                                 <span className="text-blue-400">[{new Date(log.createdAt).toISOString().split('T')[1].slice(0,-1)}]</span>
                                 <span className={log.level === 'WARN' ? 'text-yellow-400' : log.level === 'ERROR' ? 'text-red-400' : 'text-white/80'}>{log.source}</span>
                                 <span className="text-white">{log.message}</span>
                             </div>
                         ))}
                         {logs.length === 0 && <span className="text-white/20">Waiting for telemetry...</span>}
                     </div>
                  </motion.div>
                )}
                ...
                {activeTab === 'scanner' && (
                  <motion.div key="scanner" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-8 flex flex-col items-center justify-center min-h-[500px] gap-12">
                    <div className="text-center space-y-4 max-w-sm">
                       <h3 className="text-2xl font-black uppercase italic tracking-tighter">Field Sync Authorization</h3>
                       <p className="text-sm text-[#A3A6AF] font-sans">Scan this secure endpoint to map a mobile hardware node to your current terminal session.</p>
                    </div>
                    ...
                    <div className="w-80 h-80 border-2 border-dashed border-[#e0ff00]/30 rounded-[3rem] flex items-center justify-center relative bg-black/50 backdrop-blur-md p-10">
                        ...
                        <div className="bg-white p-4 rounded-2xl shadow-2xl">
                           <QRCodeSVG 
                              value={`whale-terminal-sync-${isConnected ? 'auth' : 'guest'}`} 
                              size={200} 
                              level="H"
                              fgColor="#000000"
                              bgColor="#ffffff"
                    ...
                    <div className="flex items-center gap-4 text-[10px] font-mono text-white/30 uppercase tracking-[0.4em] animate-pulse">
                       <Activity size={14} /> session_ready &bull; link_pending
                    </div>
                  </motion.div>
                )}
...
// Placeholder hook for icons not imported implicitly
